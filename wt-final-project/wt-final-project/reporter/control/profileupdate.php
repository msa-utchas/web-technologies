...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($flag == 1) {
        if (checkReporterExists($email) && $_SESSION["email"]!=$email) {
            $validateEmail = "Already has an reporter with this email";
        } else {
            $address = $street . "|" . $post . "|" . $country;
            if (isset($_POST['update'])) {

                updateReporterExists($name, $email, $password, $gender, $date, $phone, $address, $profile);
