function validateForm() {
    ...
    if (
        ...
        ex=/^[0-9a-zA-Z_' ]*/;
        if (name.length < 5) {
            document.getElementById("nameerror").innerHTML = "username should contain atleast 5 letters";
            alert("username should contain atleast 5 letters");
            flag = false;
        } else if (!name.match(ex)) {
            document.getElementById("nameerror").innerHTML = "use only a-z, A-Z, -, ', @, #, & and * those characters at name field";
            flag = false;
        } else {
